const std = @import("std");
const tui = @import("./tui.zig");

/// Pauses execution and asks for terminal approval.
pub fn askPermission(action_desc: []const u8, auto_yes: bool) !bool {
    var stdout_buf: [1024]u8 = undefined;
    var stdout_writer = std.fs.File.stdout().writer(&stdout_buf);
    const stdout = &stdout_writer.interface;
    const stdin_file = std.fs.File.stdin();

    if (auto_yes) {
        try stdout.print("\n{s}⚠️  [BLAST RADIUS] Auto-approved dangerous action due to -y flag:{s}\n-> {s}\n", .{ tui.red, tui.reset, action_desc });
        try stdout.flush();
        return true;
    }

    try stdout.print("\n{s}⚠️  [BLAST RADIUS] Sylvia requested a dangerous action:{s}\n", .{ tui.yellow, tui.reset });
    try stdout.print("{s}-> {s}{s}\n", .{ tui.yellow, action_desc, tui.reset });
    try stdout.print("{s}Allow this action? [y/N]: {s}", .{ tui.yellow, tui.reset });
    try stdout.flush();

    var buf: [16]u8 = undefined;
    const bytes_read = try stdin_file.read(&buf);

    if (bytes_read > 0) {
        const line = buf[0..bytes_read];
        const trimmed = std.mem.trim(u8, line, "\r\n ");
        // Accept 'y', 'Y', 'yes', 'Yes', etc. — treat any input starting with 'y' or 'Y' as affirmative.
        if (trimmed.len > 0 and (trimmed[0] == 'y' or trimmed[0] == 'Y')) {
            return true;
        }
    }

    return false;
}
